...
import { CheckCircle2, CreditCard, Camera, MapPin, LogIn } from 'lucide-react'
...
export default function CheckinPage() {
  ...
  const capturarGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        setLat(pos.coords.latitude); setLng(pos.coords.longitude); setAcc(pos.coords.accuracy)
        setStep('confirmacao')
      },
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/checkin', {
        ...
        body: JSON.stringify({ tipoRegistro: 'checkin', cpf, eventoId, latitude: lat, longitude: lng, accuracy }),
      })
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-amber-500 text-white px-5 pt-12 pb-6 shadow">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
            <LogIn className="w-5 h-5" />
          </div>
          <div>
            <h1 className="font-black text-lg">Check-in</h1>
            <p className="text-amber-100 text-xs">{evento?.titulo ?? 'Carregando…'}</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 px-4 py-5 max-w-lg mx-auto w-full space-y-4">
        ...
        {step === 'form' && (
          <div className="space-y-4 animate-slide-up">
            ...
            <Button size="lg" onClick={() => { if (!cpf.trim()) { setErro('Informe seu CPF.'); return } iniciarCamera() }}>
        ...
        {step === 'camera' && (
          <div className="space-y-3 animate-slide-up">
            <p className="text-sm font-semibold text-slate-600 text-center">Tire uma foto sua para validar a presença</p>
        ...
        {step === 'gps' && (
          <div className="flex flex-col items-center gap-4 py-12 text-center animate-slide-up">
            ...
            <p className="text-xs text-slate-400">Mantenha o GPS ativo</p>
          </div>
        )}
        ...
        {step === 'confirmacao' && foto && (
          <div className="space-y-4 animate-slide-up">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 text-sm">
              ...
              <div className="flex justify-between mt-1.5"><span className="text-slate-400 text-xs">Tipo</span><span className="font-semibold text-xs">Check-in</span></div>
            </div>
            {erro && <p className="text-sm text-red-500 bg-red-50 border border-red-100 rounded-2xl px-4 py-3">{erro}</p>}
            <Button size="lg" loading={loading} onClick={handleSubmit}>
              <LogIn className="w-5 h-5" /> Confirmar check-in
            </Button>
          </div>
        )}
      </main>
